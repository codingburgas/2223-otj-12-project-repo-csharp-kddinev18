...
namespace LocalServer.BLL.DataManipulation.BLL
{
    public static class PermissionModifierLogic
    {
        ...
        {
            Guid roleId = new Guid(DatabaseInitialiser.Database.Tables.Where(Table => Table.Name == "Roles").First()
                .Select("Name", "=", roleName).Rows[0]["Id"].ToString());
            Guid deviceId = new Guid(DatabaseInitialiser.Database.Tables.Where(Table => Table.Name == "Devices").First()
